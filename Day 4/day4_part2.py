...
draw_number = 0
score = 0
while len(boards) >0: #While there are boards that haven't won yet
    for bingo_board in boards: #Draw a number and check each board for bingo
        if bingo_board.bingoNum(int(draws[draw_number])): #If there is a bingo
            won_boards.append(bingo_board) #Append the board to the winning boards
    for bingo_board in boards: #Remove any boards that have won from the group that hasn't won yet
        if bingo_board in won_boards:
            boards.remove(bingo_board)
    draw_number+=1
    
#Print answer
print('Bingo Answer: ',won_boards[-1].score()) #The last board to win
