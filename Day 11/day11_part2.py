...
syncStep = 0 #Current step
while flashCount <100: #while not all the octopus flashed at the same time
    ...
    for rowIndex in range(len(grid)):
        for pointIndex in range(len(grid[rowIndex])):
            if grid[rowIndex][pointIndex] not in flashed and grid[rowIndex][pointIndex] >=10 :#If at energy level 10 and not flashed yet
                    ...
                    flashed.append(tuple([rowIndex,pointIndex])) #Add that octopus to list of already flashed
                    flash(rowIndex, pointIndex,flashed)
    #Set flashCount to the amount of flashes
    flashCount = len(flashed)
    syncStep +=1
#Print Answer
print('First syncronized flash',syncStep)